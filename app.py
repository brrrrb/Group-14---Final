...
def business_post_form():
    if request.method == "POST":
        ...
        if file and file.filename:
            if (
                "." in file.filename
                and file.filename.rsplit(".", 1)[1].lower() in Extensions
            ):
...
def add_comment(post_id):
    ...
    return redirect(
        url_for(
            "view_personal_post" if post.type == "personal" else "view_business_post",
            post_id=post_id,
        )
    )
...
def edit_comment(post_id, comment_id):
    ...
    if not edited_content:
        ...
        return redirect(
            url_for("show_edit_comment_form", post_id=post_id, comment_id=comment_id)
        )
...
def discover_page_selected():
    ...
    personal_posts = (
        Post.query.filter_by(country=selected_country, type="personal")
        .order_by(Post.posted_at.desc())
        .limit(2)
        .all()
    )
    business_posts = (
        Post.query.filter_by(country=selected_country, type="business")
        .order_by(Post.posted_at.desc())
        .limit(2)
        .all()
    )
    return render_template(
        "discover_page_selected.html",
...
def itinerary_creation_step_2():
    ...
    user_itinerary = itinerary_repository.create_itinerary(
    ...
    return redirect(
        url_for("add_activities", itinerary_id=itinerary_id, day_number=day_number)
    )
...
def add_activities_post(itinerary_id, day_number):
    ...
    if day_number < itinerary.days:
        return redirect(
            url_for(
                "add_activities", itinerary_id=itinerary_id, day_number=day_number + 1
            )
        )
    else:
...
def edit_itinerary(itinerary_id):
    ...
    return render_template(
        "edit_itinerary.html", itinerary=itinerary, countries=countries
    )
